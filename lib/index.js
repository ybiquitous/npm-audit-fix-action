...
async function run() {
  ...
  const targetDirs = await core.group("Resolve input paths", async () => {
    ...
    if (excluded.length > 0) {
      core.info(`No package files found in directories: ${patternsToText(excluded)}`);
    }

    core.info(`Target directories: ${patternsToText(resolved)}`);
    return resolved;
  });
  ...
  for (const targetDir of targetDirs) {
    await core.group(`Process directory: "${targetDir}"`, async () => {
      process.chdir(targetDir);
      const { report, files: newFiles } = await processDir();
      reports.push(report);
      files.push(...newFiles.map((file) => nodePath.posix.join(targetDir, file)));
    });
  ...
  if (report.packageCount === 0) {
    ...
    await core.group("Merged reports", async () => {
  ...
  if (files.length === 0) {
    ...
    await core.group("All changed files", async () => {
