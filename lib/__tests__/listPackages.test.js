...
test("listPackages()", async () => {
  ...
  /** @param {string} pkg */
  function assertVersion(pkg) {
    assert.match(String(packages.get(pkg)), /^\d+\.\d+\.\d+$/u);
  }

  assertVersion("eslint:eslint");
  assertVersion("@actions/core:@actions/core");
  assertVersion("npm-package-arg/node_modules/lru-cache:lru-cache");
});
