...
test("resolves literal paths as-is", async (t) => {
  const paths = await resolveDirPaths([".", "packages/foo", "packages/baz"], baseDir);
  t.assert.deepEqual(paths, {
    resolved: [".", "packages/foo"],
    excluded: ["packages/baz"],
    failed: [],
  });
});
...
test("expands a glob pattern to matching directories and ignores files", async (t) => {
  ...
  t.assert.deepEqual(paths, {
    resolved: ["packages/bar", "packages/foo"],
    excluded: ["packages/baz"],
    failed: [],
  });
...
test("throws for a pattern matching nothing", async (t) => {
  ...
  t.assert.deepEqual(paths, {
    resolved: ["."],
    excluded: [],
    failed: ["no-such-dir", "no-such-dir/*"],
  });
});

test("returns an empty array for an empty pattern list", async (t) => {
  t.assert.deepEqual(await resolveDirPaths([], baseDir), {
    resolved: [],
    excluded: [],
    failed: [],
  });
