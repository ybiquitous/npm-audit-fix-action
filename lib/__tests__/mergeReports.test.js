...
test("mergeReports() merges multiple reports into one", (t) => {
  /** @type {Report} */
  const reportB = {
    added: [],
    removed: [
...
test("mergeReports() dedupes the package count across reports", (t) => {
  /** @type {Report} */
  const report = {
    added: [{ name: "arrify", version: "2.0.1", location: null }],
    removed: [],
  ...
  t.assert.equal(mergeReports([report, report]).packageCount, 1);
});

test("mergeReports() returns an empty report for an empty list", (t) => {
